...
def parse_fn(line_before, line_after):
    ...
    source = [c for c in line_before.decode('utf-8').rstrip('\n')]
    t = [w for w in nltk.word_tokenize(line_after.decode('utf-8').strip())]
    learn_target = t + [u'<eos>'] + [u'<pad>']
    target = [u'<go>'] + t + [u'<eos>']
...
def input_fn(data_file, params=None, shuffle_and_repeat=False):
    ...
    defaults = ((u'<pad>', 0), (u'<pad>', u'<pad>', 0))
...
if __name__ == '__main__':
    ...
    hook = tf.contrib.estimator.stop_if_no_increase_hook(estimator, 'acc', 3000, min_steps=10000, run_every_secs=120)
